...
interface ArbInference {
    ...
    function inferCallPipeline(bytes memory model, bytes memory pipeline, bytes memory seed, bytes memory input) external returns (bytes memory);
    function inferCallPrivate(bytes memory ip, bytes memory model, bytes memory input) external returns (bytes memory);
}